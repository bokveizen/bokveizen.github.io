...
## Fanchen Bu CV Workspace

This folder is self-contained for the CV source and template files. Edit
`resume.tex`, then run:
...
The generated PDF is `CV_Fanchen_Bu_latest.pdf`. Use `make clean` to remove
LaTeX build byproducts while keeping the PDF, or `make distclean` to remove the
PDF as well.
